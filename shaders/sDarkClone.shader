...
void main()
{
    ...
    if(alpha == 0.0 && current_alpha > 0.0) {  // Is outline
        gl_FragColor = vec4(u_outline_color, texture2D( gm_BaseTexture, v_vTexcoord ).a);
        ...
        gl_FragColor = vec4(1.0, 1.0, 1.0, texture2D( gm_BaseTexture, v_vTexcoord ).a) * texture2D( u_inner_texture, fract((v_vTexcoord * vec2(800.0 / u_tex_size, 608.0 / u_tex_size)) - tex_offset));
    }
}
